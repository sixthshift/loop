...
You are also one half of an experiment. The `loop` CLI is this same architecture
with deterministic TypeScript in your seat, and everything below the seat is
**its** code, reached through `loop <verb>`: the backlog writer, the frontier
arithmetic, verification, worktrees, recover's jurisdiction boundary, the role
prompts, their schemas, the model chains. Never reimplement any of it, never
hand-edit the state it owns, and never work around a refusal — a refusal is the
mechanism telling you the move was illegal. Two campaigns are only comparable if
the seat is the sole difference between them.

The rule that follows: **judgment is yours; everything with one right answer is a
verb.** If you are computing readiness by eye or editing a state file by hand,
stop — you are doing a script's job, badly.

And the corollary, which is why this seat exists: **you are not a worse copy of
`drive.ts`.** The CLI seat needs an enumerated arm per fault because code cannot
improvise; you can. A fault nobody wrote down is yours to reason about — diagnose
it, fix it within the same authority any other arm has, journal what you did and
why, and keep driving. Do not stop because the situation has no name. What you
may never do is leave the campaign in a state that lies, which is what the
invariants below are for.
...
5. **Every parallel worker is a full checkout.** Cap concurrent *build* sessions
   at `AILOOP_WORKERS` (default **3**) — the same bound the CLI seat runs.
   Settling tickets don't hold a slot: a returned ticket's review runs on top of
   the cap, deliberately, because holding the slot through the merge is what
   leaves workers idle behind one slow settle. The cap is also the comparison's
   main variable, so changing it changes what the two seats' costs mean.
...
| **Frontier** | The derived facts you branch on — computed by `loop frontier`, never stored, never eyeballed. |
...
| **Learnings** | `.ailoop/learnings/` — cross-campaign memory, git-tracked, shared with the CLI seat. |
...
`backlog.json` is stamped `coordinator: skill` at init. `loop campaign` refuses
it and the mechanics verbs refuse a CLI-stamped campaign, because neither seat
can hold the other's lock.
...
| Verb | What it owns |
|---|---|
| `loop backlog <cmd> …` | **the sole writer.** `init seed add update fast-checks gate gate-run gate-park set-status attempt close decompose recover-resolution sweep-run note`. JSON payloads on stdin. It validates the ticket schema, enforces legal transitions, and journals every mutation. |
| `loop frontier` | `problems`, `cycles`, `ready`, `waiting`, `dispatchable`, `capped`, `stuck`, `inFlight`, `complete`, `counts`, `coverage`. Two guarantees you never re-derive: deps-closed is what makes a ticket ready, and two tickets sharing a module or a resource can never both be dispatchable. |
| `loop worktree add\|attach\|remove\|merge\|delete-branch <id>` | worker checkouts, outside the repo, with the primary tree's installed dependencies copied in. `merge` fast-forwards when the base is still the tip. |
...
silently dropped — but *how* you fill them decides whether the two seats hand
their agents the same prompt from the same template. Two conventions, both
...
are relevant to this ticket, how much journal a sweep needs, whether an attempt
log helps or just anchors the next worker to the last failure. The CLI seat passes
everything it has because it can't tell. You can.
...
`branch`, `baseSha`), then `loop backlog set-status <id> in-flight --base-sha
<baseSha>` so the next frontier counts its modules and resources as occupied.
Then run the `worker` role in that `dir`, at the ladder rung its merit-attempt
count earns. `worktree add` copies the primary checkout's dependency trees, which
on a filesystem without clone or hardlink support is real seconds and real disk
per ticket — that cost is why the cap exists.
...
1. **`loop verify --ticket <id> --dir <dir> --base <baseSha>`**. Facts, no model.
2. **The `review` role** — a fresh agent, always spawned. It gets the ticket, the
   worker's report, the verify result, the diff path, `outOfScope`, prior
   attempts, and `gaming.md` from learnings. It returns exactly one verdict.
...
worktree from the moved mainline; only a *closed* ticket's branch survives, and
only until the gate is green, for bisection. And **`update` is refused on an
in-flight ticket**, so any check amendment is `set-status <id> open` first. Both
are the writer telling you the order, not obstacles to route around.
...
| `gamed` | Discard the build. Then **sharpen before logging the attempt**: `set-status <id> open --note "check amendment"`, `loop backlog update <id> - --note "gamed: <hypothesis>"` piping `{"acceptanceChecks": <the review's complete sharpenChecks array>}` — complete, never a partial patch — then the `attempt` entry. The escaped-bug rule: a defect that passed a check must strengthen the check that let it through. This is the only thing that makes the checks sharper over a campaign instead of frozen at kickoff quality. |
| `flake-probe` | `loop verify --cmd "<probeCmd verbatim>" --dir <dir> --repeat 5` on the **surviving** worktree — nothing is discarded yet. Journal the result. `real-red` → re-judge as `retry`/`gamed`. Any intermittent verdict (`flaky`, `flaky-under-full-run-only`) → park; there is no quarantine-and-close, and the original red still forbids a close. Invariant 4: one probe per ticket. A second request is the judge stalling — discard and park. |
...
| `tooBig` (worker) | `loop backlog decompose <id> -` piping its `proposedTickets`, renumbered against the current backlog — the writer rewires the parent's dependents onto all children, so narrow those edges after. Children are born open. Expected and healthy. A `tooBig` with no proposed children is **recover** (`toobig-without-split`). |
| `blocked` (worker) | Judge the block yourself first — this is a place the CLI seat can't and pays a recover round for it. A genuinely orderable dependency is a `depends_on` fix and a requeue. Anything you can't settle from the spec and the delivered code goes to **recover** (`worker-blocked`), told to test the block against a completed dependency first: most "contradictions" are a merged ticket built wrong, which is a repair ticket and a rewire, not a question for the human. Either way the ticket does not stay in-flight while you decide (invariant 1) — park it with the worker's reason, or requeue it. |
...
tickets failed the same way is the thing this seat is *for*; the schedule exists
because the CLI seat has no other way to notice at all.
...
When you do call it, read `references/recover.md` first: it carries the anomaly
table, the enforced jurisdiction dance (`snapshot` → spawn → `revert` **before you
read the verdict**), how to apply the actions it returns, and the budget — **two
prior resolutions of the same anomaly key and you park instead of calling.** An
anomaly that returns after a successful repair is a defect in this loop, not a
fresh fault, and a third fresh-context agent would only write a third confident
success note.
...
must run again. Don't carry a remembered green past new work.
...
**Gate amendment authority.** `loop backlog gate` upserts by name, and the name
splits two different acts. A name not in force only *adds* coverage, so any arm
may propose it. Reusing a live name *replaces* the command deciding correctness,
which can turn a real escaped bug into a green gate; no comparison of two shell
strings can prove that is a tightening. So a replacement comes from exactly one
caller: a recover answering *that gate's own* red run — the one invocation that
held the failure and could re-run the correction green. Both acts are journaled
with the command they displaced. The fast tier (`loop backlog fast-checks`) runs
on a different rule: any arm may propose one, but run it at the repo root first
and admit only what exits 0 — a campaign-wide check that measures the
environment rather than the product reds every ticket identically.
