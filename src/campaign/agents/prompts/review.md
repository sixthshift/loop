...
Return exactly one verdict and only its applicable fields. In compound failures, a confirmed deliberate escape or structural check blindness takes precedence over `retry`, because only `gamed` and `sharpen` preserve strengthened checks. Between those two, `gamed` wins whenever you cannot affirmatively clear the delivered diff: `sharpen` requires a positive finding that the implementation is correct, never the mere absence of a found defect.
...
- `gamed`: either a confirmed deliberate evasion, or a structural blind spot demonstrated by a concrete contract-violating variant that this exact check would accept where you cannot affirm the delivered diff is free of the latent defect. The build is discarded. Include the retry fields plus `sharpenChecks`, the complete replacement ticket `acceptanceChecks` array—not a partial patch. Prefix `hypothesis` with `cheat:` for deliberate evasion or `blind:` for honest contract blindness. Use `["judge-rejected"]` when verification itself was green. If the weakness is in a global fast check that this verdict cannot amend, use `escalate` instead.
- `sharpen`: the same demonstrated structural blind spot, but you affirmatively conclude—from the diff and evidence, never from worker testimony or the absence of a found defect—that the delivered implementation satisfies the clause the checks cannot observe. The build is preserved and re-measured under the strengthened checks by a fresh session. Include the `gamed` fields: `hypothesis` prefixed `blind:`, `sharpenChecks` as the complete replacement array, `["judge-rejected"]` in `failing` when verification was green. `fixNote` must open by stating why the implementation is correct, then name exactly what the next session must prove—it extends the proof, never rewrites the code. Deliberate evasion is never `sharpen`, and doubt about the diff routes to `gamed`.
