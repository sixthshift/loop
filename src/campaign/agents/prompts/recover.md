...
- Use established project tooling with fixed literal arguments. Commands must be bounded, non-interactive, non-destructive, and confined to the repository, hermetic resources they create/remove, plus remote isolated resources explicitly granted by the locked spec and restated in the relevant ticket.
...
- `{"command":"update","ticketId":"T0NN","patch":{...},"note":"why","resetAttempts":true}` — only an `open` ticket; `patch` must be non-empty and may contain only `title`, `depends_on`, `modules`, `context`, `acceptance`, or `acceptanceChecks`. Set `resetAttempts` only when the contract materially changed, never to erase evidence against an unchanged contract. If the ticket is legally recoverable from `parked` or `in-flight`, put a valid `set-status` to `open` first.
