...
- `milestones`: the spec's declared checkpoints as `[{id, name, delivers}]`, where `delivers` cites the requirement ids above. Take them from a `## Milestones` section if the spec has one and translate its cited requirement headings into your ids; return `[]` when it declares none. **Never invent a milestone.** They order nothing and gate nothing — a milestone marks the moment a slice of the product exists, and the campaign's reflective sweep runs when one is reached. Inventing one moves that moment to a place the human never claimed the product would be coherent. A milestone whose `delivers` names a clause you did not enumerate is refused by the writer, so cite only ids you returned.
- `caps`: the spec's declared spend policy as `{maxAttempts, thrash}` (optionally `infraCap`), or `null` when it declares none. Take it from a `## Spend policy` section if the spec has one; **never invent one**. It decides how many times a ticket may fail on its own merits before the campaign stops and asks the human, so a number you supplied rather than read would be the loop authorizing its own budget. `null` leaves the campaign's conservative defaults in force.
