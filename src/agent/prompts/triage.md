...
- `{"command": "gate", "gates": [{"name": "...", "cmd": "..."}], "note": "why"}` — amend the campaign's merged-tree gate (upsert by name). When a spec-mandated invariant is proven only per-ticket and never re-verified after merge, adding it to the campaign gate is the escaped-bug rule in action — do this, don't escalate.
